...
Francas Industriales y de Comercialización"); LSI interface = **14_**
(D.L. Nº 431, 11-oct-2007, D.O. N° 199 T.377 25-oct-2007; 2007 print,
header "Reformas: S/R"); customs-chassis kin = **74_** (D.L. Nº 529,
...
75 (exports), 76 and 77 (refund/credit regime) ride INSIDE the 12_/14_
...
consolidation ends at D.L. 318-2013 and 14_/74_ are the 2007/2012-endside
prints — post-cutoff reforms are unverified until official routes recover
(SOQ-22 kin; D.L. 598-2020-era traffic may have touched the regimes);
article text is cited **as printed** (12_ prints Arts. 21 onward in
UPPERCASE; 14_/74_ print sentence case — casing preserved). Verbatim text
below is copied from the W13 evidence files (EVID-252/257/261/263/270) and,
...
- **SV-SPE-FR-094:** The system shall implement the ZF/DPA disposition
  ...
  12_ Art. 27 ZF/DPA paths, DISTINCT from the LSI 14_ Art. 45 residuales
  paths owned by `04_customs-clocks.md` (SV-SPE-FR-079 by id — different
  instrument, different regime, never merged).
  (LB-004; EVID-257)

### 3.5 LSI export test + local-market routing (14_ Art. 8)
...
### 3.6 LSI dual-retention pair (14_ Art. 8)
...
- **SV-SPE-FR-097:** The system shall carry the LSI 1.5% ISR-anticipo
  ...
  services of letters a), b), c), d), e), h) and j), the PRESTATARIO
  ...
  id); the 1.5% value is the dated 2007 print (SOQ-37 cross-check OQ-1);
...
- **SV-SPE-FR-098:** The system shall carry the LSI 1% IVA-anticipo
  ...
  services of letters a)-e), h) and j), ALL IVA contribuyentes regardless
  of classification receiving the service MUST retain 1% of the VALUE OF
  THE SERVICE RECEIVED as IVA *anticipo* (this file owns the trigger
  predicate and the 1% dated row; the retention DOCUMENT is the
  e-invoicing CRE consumed BY ID — SV-EINV-FR-035, CAT-006 code 22
  "Retención IVA 1%"); the 1% value is the dated 2007 print (SOQ-37
  cross-check OQ-1); dictamen verification hook as in FR-097.
  (LB-005; EVID-261)

### 3.7 Cross-regime services — the FCF rule (14_ Art. 8 inc. 2)
...
### 3.8 LSI goods interface (14_ Arts. 8/26)
...
| l10n_sv_special_regime.fiscal_route | valid_from · valid_to · provenance | date · date · char | 12_/14_ article as printed (SOQ-30); snapshot-on-write per D15 | FR-087..101 |

**LSI retention applicability (l10n_sv_special_regime.retention_rule):**

| Entity | Field | Type | Catalog / values | Reference |
|--------|-------|------|------------------|-----------|
| l10n_sv_special_regime.retention_rule | kind · letters | select · m2m | isr_anticipo_1_5 (a,b,c,d,e,h,j) · iva_anticipo_1 (a,b,c,d,e,h,j) — dated 2007 values, SOQ-37/OQ-1 | FR-097, FR-098 |
...
| l10n_sv_special_regime.retention_rule | valid_from · valid_to · provenance | date · date · char | 14_ Art. 8 as printed (2007) | FR-097, FR-098 |
...
| FR-094 | odoo | l10n_sv_special_regime.disposition_path | 4 ZF/DPA paths | valor en aduana / botaderos free / CAUCA destruction / donations MINEC+Asamblea; DISTINCT from LSI FR-079 (14_ Art. 45) — never merged |
...
| FR-097 | odoo | l10n_sv_special_regime.retention_rule + account.tax | isr_anticipo_1_5 rows | applicability HERE; engine = SV-TAX-FR-121..129 by id; CT-anticipo suppression flag; 2007 print value (OQ-1); dictamen hook = 03 FR-053/054 by id |
| FR-098 | odoo | l10n_sv_special_regime.retention_rule + account.tax | iva_anticipo_1 rows | all IVA contribuyentes on receiving side; CRE doc = SV-EINV-FR-035 by id (CAT-006 22); 2007 print value (OQ-1) |
...
|----|----------|-----------|-------|--------|
| OQ-1 | SOQ-37 carried: the LSI retention values 1.5% ISR anticipo / 1% IVA (14_ Art. 8) are the DATED 2007 PRINT — verify no later CT (Arts. 157/158 family) or Ley-ISR reform superseded them; cross-check against the 54_ consolidated text and CT at the taxation cross-check (origin: EV14 OQ-2). FR-097/FR-098 ship them as dated rows with 2007 provenance; a reform lands as new dated rows. | no | Takumi S7 (taxation cross-check) | open |
...
| OQ-3 | SOQ-30 carried: the 12_ consolidation ends at D.L. 318-2013 and 14_/74_ are the 2007/2012-endside prints — post-cutoff reforms unverified until official routes recover (D.L. 598-2020-era traffic may have touched the regimes, the 0%/13% routing assumptions or the retention pair); every LB in this file cites as printed — re-verify before implementation. The Ley IVA Art. 54/75-77 anchors additionally ride the future IVA-core wave's own verification. | no | Takumi S7 (sources watch) | open |
