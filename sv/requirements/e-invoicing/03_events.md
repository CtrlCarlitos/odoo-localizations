...
- **SV-EINV-FR-094:** The system shall enforce the event timestamp windows: for retorno, the declared event date may be up to 5 days after the transmission anchor of Anexo V N°9 rule 2 ("la fecha del evento sea hasta 5 días posteriores a la fecha de transmisión del mismo" — raw verified; the source's anchor noun is ambiguous between the event's own transmission and the related DTE's, Retorno-scoped rule; verify against AT guidance) without crossing into the next tax period, and the emission-time fields accept a 30-minute holgura on the last day of a month (per-event field names per schema: invalidación/retorno/OpEsp use fecEmi/horEmi, contingencia uses fTransmision/hTransmision). (LB-007 N°9/11/12; LB-010; 45_ raw p.123)
...
- **SV-EINV-FR-103:** The system shall enforce the differentiated invalidation deadlines, measured from the target's seal: CCFE, CRE, NCE and NDE ⇒ 1 day; CLE and DCLE ⇒ the first 10 días hábiles of the month following the settlement period; NRE, CDE, Evento de Retorno and Evento de Operaciones Especiales ⇒ 4 calendar days; FE, FEXE and FSEE ⇒ 3 months — for FE and FEXE only (not FSEE) extendable to 2 years when the related DTE's emitter actividad-económica code is one of {21001, 21008, 46482, 46484, 46491, 47721}, the 2-year rule covering both invalidation and retorno transmission (Anexo V N°9 rule 7 — OQ-002 resolved). (LB-007 N°9 + N°43–48; DG45 §3.2; R6)
- **SV-EINV-FR-104:** The system shall enforce the event-date correspondence rules of Anexo V N°9: when invalidating a DTE other than FE/FEXE/FSEE (or an event), the event's fecEmi shall equal the target's generation/event date (rule 4); for FE/FEXE/FSEE targets the event date may be up to 3 months after the target's generation date (rule 5) and its transmission up to 3 months after the target's seal (rule 6). (LB-007 N°9; DG45 §3.2; OQ-002 resolved)
...
| l10n_sv_edi.event (client mirror) | deadline_at / deadline_kind | datetime / select | invalidación group · contingencia 24h/72h · retorno 3mo (2y FE/FEXE activity codes) /1d · OpEsp 10-hábiles | FR-103/111/112/117/126 |
...
|----|----------|-----------|-------|--------|
| OQ-001 | Contingencia `version`: Anexo V N°1 (45_, May-2026) prints 3, but the 52_ schema (Aug-2026, later authority) sets const 4 — requirements adopt 4 (FR-088). Master-index R7 ("contingencia 3 ... confirmed by 52_") needs amendment. Confirm no Normativa/schema cycle mismatch at implementation. | no | Takumi (schema pass) | **resolved** — contingencia-schema-v4.json `version.const = 4` re-verified in the 2026-08-17 pass; R7 already amended in the master index (S1 ruling 3). Normativa v2.0 (May) prints 3, the later schema set (Aug) sets 4 — schemas govern per authority order; no cycle mismatch |
| OQ-002 | Anexo V N°9 scope (OCR-mixed): does the 2-year window for emitter activity codes {21001, 21008, 46482, 46484, 46491, 47721} attach to invalidation of FE/FEXE/FSEE (written FR-103), to retorno transmission windows, or both? Likewise the "same date as the invalidado" correspondence (FR-104) and which date fields it binds — the invalidación schema carries fecEmi/horEmi (no fecEmiEvento). Confirm from 45_ original tables or AT guidance. | no | Takumi synthesis wave 1 | **resolved** — 45_ raw text pp.123–124 (Anexo V N°9 rules 4–8, legible): the 2-year window (rule 7) binds **both invalidación and retorno**, for **FE/FEXE only** (not FSEE), measured from the related DTE's transmission (seal) date, conditional on the DTE's emisor "Código de Actividad Económica". Rule 4: same-date rule = event's Fecha del evento equals the target's "Fecha de Generación"/"Fecha del evento" when invalidating non-FE/FEXE/FSEE docs or events. FR-103/104/117 amended accordingly |
| OQ-003 | Anexo V N°48 codigoGeneracionR null-type list prints "05, 08, 16, 17, 18" under a code scheme conflicting with CAT-002 v1.1 (CLE=08/DCLE=09/CDE=15 vs 15/16/17 in DG45 §3.3 parentheticals — see `01_document-types.md` OQ-008); schemas encode tipoDte as free string so the business rule is not machine-checkable. Confirm the exact null-set (presumably NCE + liquidation/donation types + events). | no | Takumi (schema pass) | **resolved** — raw 45_ genuinely prints "053, 08, 16, 17 y 18" (053 = 05 erratum per DG45 §5). CAT-002 v1.1 has **no code 16** and 52_ schemas pin CLE=08/DCLE=09/CDE=15 (01 OQ-008) → null-set = {05 NCE, 08 CLE, 17 OpEsp, 18 Retorno} for tipoInvalidación 2 and for those types under 1/3 (2022 base {NC, CL} + the new event types, which have no replacement DTE); "16" is a ghost code with no catalog referent — treat as erratum pending AT confirmation. Schema `documento.tipoDte` is a free string (not machine-checkable), so the SaaS must encode the set itself |
...
| OQ-005 | Wire keys vs Anexo III unified names: 52_ schemas use `tipoOperacion` (eret), `motivoContin`, `tipoAnulacion`/`motivoAnulacion`, `fTransmision`/`hTransmision`, `fusion`, `ventaTercero.codDomiciliado`, `docDel`/`docAl` — diverging from the Anexo III digest names (tipoTransmision, tipoInvalidacion, fecEvento, fusiones, domicilioFiscal, DocDel/DocAl). Schemas govern (noted in §4); confirm the mapping table in the SaaS schema pass so a future Normativa bump renaming keys is caught by the catalog-version SLA. | no | Takumi (schema pass) | **resolved** — 2026-08-17 pass confirms every wire key in the 52_ schemas: tipoTransmision→`tipoOperacion`; tipoInvalidacion→`tipoAnulacion` (CAT-024 values); fecEvento→`fecEmi`/`horEmi` (invalidación, eret, eop) and `fTransmision`/`hTransmision` (contingencia); fusiones→`fusion`; domicilioFiscal→`ventaTercero.codDomiciliado`; DocDel/DocAl→`docDel`/`docAl` (eop cuerpo); plus N°76→`codTributo`, N°31→`emisor.tipoItemExpor` (eret), N°92→`resumen.totalCompraExcluidos` (eret). Schemas govern; keep this table as the rename-tripwire input for the catalog-version SLA |
| OQ-006 | MOQ-05 (cross-ref `02_transmission.md` OQ-003): Retorno/OpEsp endpoint paths still unpublished — FR-090 depends on the connector integrating them when the AT publishes the services. | no | SaaS team | open — 52_ schema pass 2026-08-17 verified zero endpoint strings in all 15 schema files (see `02_transmission.md` OQ-003) |
