...
- **SV-PROT-FR-023:** The SaaS shall, before any transmission, verify the returned JWS against the emitter's PUBLIC certificate AND verify that the payload segment decodes to bytes matching `payload_sha256`; on success it shall assemble the signed artifact byte-stably from the SAME bytes it sent (`04_signing_delivery.md` FR-153 — no re-serialization drift); on failure it shall mark the operation `verify_failed` with an `E-SIGN` finding and transmit nothing. (LB-005; LB-006; cross-ref 04 FR-132/133/153)
...
| protocol.finding | family | enum | E-AUTH · E-ENTITLEMENT · E-VERSION · E-SCHEMA · E-BUSINESS · E-ENV · E-ORDER · E-SIGN · E-MH · E-RATE · W-* | FR-019 |
...
| FR-013 | saas | — | — | Idempotency by client_request_id; client reuses id on retry |
...
| FR-019 | shared | — | family enum | Taxonomy incl. E-MH relay carrying verbatim MH response fields |
...
- **AC-007:** Given an `E-BUSINESS` finding with `field_pointer` `/cuerpo/3/precioUnitario`, when the client renders it, then the message attaches to that invoice line in the UI and to the document's findings record (FR-018, FR-020).
...
| OQ-005 | CROSS-FILE UPDATES needed from this file (controller pass, not done here per task constraints): (a) `04_signing_delivery.md` OQ-001 → status `resolved`, pointing to FR-021..024 here; (b) `01_document-types.md`, `02_transmission.md` and `03_events.md` reference this file as `07_api-contract.md` — the actual path is `06_api-protocol.md` (04 already correct); (c) 04 OQ-008 (03 FR-090 "sign… in the SaaS core" wording) already recommends its amendment, unchanged by this file. | no | Controller | open |
