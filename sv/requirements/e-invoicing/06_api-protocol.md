...
| OQ-008 | Formalize the D10 version-regime guarantee as FRs in the next 06 edit: every document/event record the SaaS returns shall carry its MH schema version (and the emission-applicable normative set identifier), so the Odoo mirror renders history without knowing MH schemas and the client never needs an MH-version upgrade. Decided in the regulatory-change session (D7/D10, 2026-08-17); wording pending. | no | Controller | open — decision taken, FR wording pending |
