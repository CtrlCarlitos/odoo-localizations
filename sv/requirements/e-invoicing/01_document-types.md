...
- **SV-EINV-FR-009:** The system shall emit `identificacion.fusion` (N°13, wire key schema-confirmed singular; digest name "fusiones"; applies to NCE, NDE, CRE, CLE) as null when no merger applies, or as a NIT of 9 or 14 digits that exists in the AT's list of merged contributors (*lista de contribuyentes fusionados*, per Art. 69 inc. 1 LST). (LB-003 N°13; LB-004 N°13; DG45 §1.3; 52_ schemas)
...
- **SV-EINV-FR-011:** The system shall emit the *domicilio fiscal* (fiscal domicile, CAT-032; wire key `codDomiciliado` on both surfaces — schema-confirmed; digest name "domicilioFiscal") in the receptor block (N°53) for CDE and CLE, and in `ventaTercero` (N°71) for FE, CCFE, NRE, NCE, NDE and FEXE; code 1 "Domiciliado" requires the third party to hold a NIT registered in IVA. (LB-003 N°53/71; LB-004 N°69–73; DG45 §1.3; 52_ schemas)
- **SV-EINV-FR-012:** The system shall enforce the structural limits per document: cuerpo maximum 2000 items (CRE and CLE: 500); documentos relacionados maximum 50 documents totaling at most 2000 items; otrosDocumentos 1–10 entries (FEXE: 1–20); apéndice 1–10 entries. (LB-004 N°74 + DG45 §1.5; R10; DCLE cuerpo is a single settlement record — cap not applicable, schema-confirmed, OQ-005 resolved)
...
- **SV-EINV-FR-022:** FE resumen shall compute subTotalVentas = totalNoSuj + totalExenta + totalGravada; subTotal = sumatoria − global discounts; montoTotalOperacion = subTotal + non-informative tributo values; totalPagar = montoTotalOperacion ± retentions/noGravado, floored at $0.00. (LB-015 §V.1a; LB-004 resumen N°139 — subtraction formula verified in 45_ raw, OQ-004 resolved; EVID-019)
...
- **SV-EINV-FR-046:** FOVIAL (D1) and COTRANS (C8) tributes in FEXE shall require prior AT authorization (*solicitud/autorización*). Identity glosses (W19, instrument-anchored — special-regimes/08 by id): D1 = the $0.20/galón *Contribución de Conservación Vial* per D.L. 208-2000 Art. 26 **as reformed by D.L. 597-2001** (the $0.20 is the 597 reform text; the original 2000 article carried no value); C8 = the $0.10/galón **contribución especial para la estabilización de las tarifas del Servicio de Transporte Público de Pasajeros Tipo Colectivo y Masivo** per D.L. 257-2021 Art. 3 — a passenger-tariff contribution (NOT a cargo-transport/"transportistas de carga" tribute; in force 23-dic-2021 through the prorroga chain to 31-dic-2026). (LB-004 N°96; DG45 §3.1; SR8 LB-007/008/009/011 by id)
...
| res.partner / sale | domicilioFiscal (NEW; wire key `codDomiciliado`) | select CAT-032 | 1 Domiciliado / 2 No Domiciliado | FR-011 |
| account.move.line | noGravado / ivaPerci / iva13 / ivaRete (NEW, NCE/NDE) | monetary | — | FR-030 |
| account.move (DTE) | codTributo (N°84 "Tributo sujeto a cálculo de IVA") | char CAT-015 §2, null | codes A8, 57, 90, A6 with tipoItem 4 mechanics | FR-018 note; OQ-002 resolved (wire key schema-confirmed) |
...
| OQ-002 | MOQ-08: exact JSON key spellings unresolvable from OCR — DTE N°84 *tributoSujetoIVA*, DTE N°7 `tipoOperacion` vs `tipoTransmision`, CRE N°103 `montoSujetoGrav`. | no | Takumi (schema pass) | **resolved** — 52_ schema pass 2026-08-17: N°84 wire key = `codTributo` (all DTE item schemas; description matches "Tributo sujeto a cálculo de IVA"); N°7 = `tipoOperacion` (schema description "Tipo de Transmisión"); CRE = `montoSujetoGrav` (fe-cr-v2 cuerpoDocumento item) |
| OQ-003 | MOQ-08 (part): NCE/NDE allowed CAT-015 codes in v2.0 — 2022 manual listed anomalous codes T1/D8/C5; confirm allowed set from 52_ schemas. | no | Takumi (schema pass) | **resolved** — schemas leave item `tributos` as free 2-char strings (no enum); Anexo IV N°96 rule 1 validates FE/CCF/NRE/NCE/NDE against CAT-015 **sections 1+3** (only FE carries the 20/C3 exclusion, rule 2); CAT-015 v1.1 contains **no T1/D8** (2022 anomaly codes dropped from the catalog) and C5 exists in section 3 (informative) — the 2022 anomaly is obsolete |
| OQ-004 | MOQ-09: global-discount sign in resumen.subTotal — 2022 FE formula shows subtraction, CCFE prose shows addition; never arbitrated. | no | Takumi (schema pass) | **resolved** — Anexo IV N°139 (45_ raw pp.118–119): subTotal = "Sumatoria de operaciones" − descuento global (no sujetas) − descuento global (exentas) − descuento global (gravadas) for FE/CCFE/NRE; FSEE = "Total de operaciones" − descuento global. **Subtraction uniformly**; the 2022 CCFE "+" prose was the copy-paste defect (R1 sibling). FR-022 confirmed |
| OQ-005 | MOQ-12: DCLE cuerpo item cap — 18_ v1.2 set 500 for CRE/DCL/CL, but 45_ digest restates only "CRE y CLE 500"; DCLE cuerpo is a single record in any case. | no | Takumi (schema pass) | **resolved** — fe-dcl-v2 `cuerpoDocumento` is a single **object** (not an array; no maxItems) holding the settlement record — item caps structurally inapplicable; 500 remains CRE/CLE only |
...
| OQ-008 | Type-code discrepancy: DG45 §3.3 parentheticals list contingency-excluded types as "CLE (15), DCLE (16), CDE (17)" but CAT-002 v1.1 (from official 51_ XLSX) codes are CLE=08, DCLE=09, CDE=15 (matching 2022 structures). Requirements use CAT-002. | no | Takumi (schema pass) | **resolved** — 52_ schemas pin `tipoDte` per file: F=01, CCF=03, NR=04, NC=05, ND=06, CR=07, CL=08, DCL=09, FEX=11, FSE=14, CD=15. CAT-002 v1.1 codes confirmed; DG45 §3.3 parentheticals (15/16/17) are errata |
