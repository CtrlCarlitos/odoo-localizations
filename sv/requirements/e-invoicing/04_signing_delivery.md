...
- **SV-EINV-FR-136:** The Odoo client shall keep an encrypted certificate vault with ONE MH-issued certificate per environment — ambiente 00 *pruebas* (test) and 01 *producción* (production), each personalized and obtained through that environment's acreditamiento — and shall bind every signing operation to the certificate whose ambiente matches the payload's ambiente, refusing cross-environment signing (a test certificate shall never sign a production payload or vice versa). (LB-008; LB-012 CAT-001; LB-013 D2)
