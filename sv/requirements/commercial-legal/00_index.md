...
| Status  | draft (S5 commercial-legal synthesis wave + W23/W27 fold-ins, in review) |
| Authors | Takumi synthesis wave 5 + controller + W23 fold-in + W27 controller |
| Updated | 2026-08-24 |
...
| [10_aml-compliance.md](10_aml-compliance.md) | D.L. 426 (71_) AML layer, C10 REBASED W12: sujetos obligados registry + 15-*días hábiles* update clock, dated-regime adaptation (15_ historical row for pre-cutover facts), compliance program (UIF registration, officer, manual), KYC/CDD/EDD + beneficiary owner, ROS/STR reporting with no-tip-off, threshold monitor as DATED CONFIG under the R28 authority chain (71_ Art. 25 delegation + 72_ Art. 51 values), record retention (FR-028 rows), and sanctions in SMM units (SV-PAY-FR-011/022 by id) with the 10y/50y prescription ladder; W28 adds the LB-031 awareness row (Res. CVPCPA 111-2026 = 128_: the Art.-58 diagnostic requirement + the express no-lineamientos statement + 12-month window corroboration, OQ-001 watch evidence) | SV-CML-FR-193..224 | 32 | 31 | 14 | 4 |
| **Total** | | SV-CML-FR-001..248 | **248** | **260** | **144** | **35** |

Numbering note: FR numbering is wave-sequential within the `SV-CML`
prefix (001-248, no gaps, no renumbering; 001-017 = C1, 018-030 = C2,
...
`../payroll/` sidecars, cited by id). W23 tails (2026-08-23):
FR-225..242 + FR-246
appended in `04` (SAS profile + 109_ acciones re-anchors + 305-Q/R
SAS-eligibility routes), FR-243..245 in
`05` (fusión transnacional + 338-escheat/358-domicilio) — no renumbering;
consumers cite by FR id. W27 tails (2026-08-24):
FR-247..248 appended in `04` (305-C accionista-not-employee +
305-I inscription-certificate proof; LB-041..043 complete the
305-A..Z citation spine incl. the 305-T awareness LB) — no renumbering;
...
- OQ-001 — New AML Reglamento + new UIF instructivo PENDING (R28 chain; 71_ Art. 56 deadlines past; supervisor side corroborated by Res. CVPCPA 111-2026 = 128_, LB-031: no lineamientos as of 27-feb-2026) — watch uif.gob.sv + D.O. (feed stall gates Mar-2026+ issues) + cvpcpa wp-json; thresholds/subjects/formats re-verify on issuance. open
