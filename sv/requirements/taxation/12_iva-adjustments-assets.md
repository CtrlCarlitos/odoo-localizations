...
SIX mandatory fields (production or import-document lot number; name and
...
- **SV-TAX-FR-287:** For the medicines class the system shall carry the
  MANDATORY detailed registry of products sold and returns, with the SIX
  ...
  por la transferencia*; 6) *fecha del comprobante de crédito fiscal* y
  *nombre del código del cliente* (CCF date and client code) — the
...
| l10n_sv.iva.lot.registry.line | ccf_move_id (number + fecha), cliente_codigo | m2o account.move / char | statutory fields 5-6 (CCF emitted by the transfer, its date; client code) | FR-287 |
...
| FR-287 | odoo | l10n_sv.iva.lot.registry.line | the 6 statutory fields | MANDATORY registry — none dropped; incompleteness blocks the 2y deduction |
...
- **AC-002:** Given a vencido perishable-medicine return at month 20
  from the entrega, with a complete six-field lot registry line, an NC
