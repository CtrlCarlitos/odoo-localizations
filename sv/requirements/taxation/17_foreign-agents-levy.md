...
**entero** of the FULL retained sum (*sin dedcción* [sic] *alguna*) to
...
with [sic], no wording altered. The Art. 13 entero sentence prints
**"sin dedcción" [sic]** (the print defect EVID-397 and the W20-A
cluster contract carry; = "sin deducción"); the extraction txt itself
renders the normalized "sin deducción" (txt line 355) — this file
quotes the PRINT reading with [sic] per the quote-fidelity rule, the
normalization noted at LB-004. **V1 citation rule:** every LB row below
...
- **SV-TAX-FR-409:** The system shall implement the entero: "Las sumas
  ...
  sin dedcción [sic] alguna" (the print defect for "sin deducción" —
  the FULL retained sum, nothing netted), "a la Dirección General de
...
- **SV-TAX-FR-411:** The system shall implement the monthly
  ...
  que sigan al del cierre del período mensual en que recibieron" — a
...
| Entity | Field | Type | Catalog / values | Reference |
|--------|-------|------|------------------|-----------|
| l10n_sv.special_levy.entero (new) | period, amount, due_window, state, form_ref | char / monetary / date-computed / select / char | full retained sum ("sin dedcción [sic] alguna"); due = first 10 días hábiles of following month (engine by id); state: pending · entered; form_ref = AT formulario (OQ-2 — empty allowed, no default) | FR-409 |
...
| FR-409 | odoo | l10n_sv.special_levy.entero | full-sum entero | "sin dedcción [sic] alguna" (print defect = sin deducción) to DGT offices + MH-authorized banks; due = first 10 días hábiles of following month on the shared engine (SV-FREP-FR-202..204 by id); AT formulario ref (OQ-2) |
...
- **AC-003:** Given June-2025 retentions (período tributario June),
  ...
  retained sum ("sin dedcción [sic] alguna" — nothing netted), and the
...
- **AC-008:** Given the LB-004 verbatim quote, then it carries "sin
  dedcción [sic] alguna" AS PRINTED with the [sic] marker and the
  normalization note (the extraction txt renders "sin deducción") —
  quote fidelity preserved and grep-verifiable (LB-004; FR-409).
