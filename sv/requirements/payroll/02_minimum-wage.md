...
- **SV-PAY-FR-013:** The system shall guarantee the per-jornada minimum
  ...
  agrícola row (café collection is expressly included in the agrícola sector
  definition; caña collection reads through the "otras actividades
  agrícolas" catch-all — LB-005). (LB-002; LB-005; LB-007; EVID-208/191/192)
...
- **SV-PAY-FR-023:** The system shall feed the 4×SMM indemnización cap of
  ...
  (forward reference by file, resolved at that file's review per the S3
  forward-reference ruling) — this FR supplies only the dated values and
...
these FRs touch DTE generation/transformation (the only architecture-split
surface per `shared/docs/saas-thin-client-architecture.md` D2). Model names
...
| FR-023 | odoo | l10n_sv.smm.sector.mapping + l10n_sv.smm.tariff | 4×SMM cap feed | CT Art. 58 daily-tariff feed; arithmetic owned by 07_contracts-termination.md (forward reference, resolved at that file's review) |
...
| OQ-002 | SOQ-18 carried: which sector's SMM feeds each UNPINNED SMM-indexed rule — the FE receptor threshold ≥3 SMM (`e-invoicing/01_document-types.md` OQ-007), the 25-SMM ISR cash-payment ban (`taxation/02_isr-deductions.md` OQ-002) and the 4×SMM indemnización cap (CT Art. 58 "salario mínimo diario legal vigente", EVID-207)? FR-022 ships configurable selection with documented defaults; MH guidance hunt pending. | no | Takumi S4 + taxation/e-invoicing consumers | open |
