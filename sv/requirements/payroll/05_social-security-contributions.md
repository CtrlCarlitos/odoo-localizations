...
| LB-009 | Ley Integral del Sistema de Pensiones, Arts. 8 and 11 (gloss-level per master index P6): Art. 8 — worker's free choice of Administradora (AFP), with employer obligatory registration within TWENTY days when the worker does not choose; Art. 11 — exclusions from obligatory affiliation: workers covered by IPSFA/IvD and the institutos case | Routing anchors: the worker chooses the AFP (employer registers within 20 days by default when no choice is exercised); workers covered by the public institutos (IPSFA/IvD) are excluded from SIP obligatory affiliation | `sv/sources/09_Ley_Sistema_Pensiones.pdf` | Arts. 8, 11 pp.4-5 (EVID-197; EVID-200) |
...
- **SV-PAY-FR-075:** The system shall compute the monthly IBC for
  ...
  benefits are OUT): the IBC builder sums the flagged lines and
