...
| **Total** | | SV-PAY-FR-001..137 | **137** | **126** | **94** | **40** |
...
Status legend: `open` unless noted `resolved`. 40 open / 0 resolved per
the per-file status columns. Index-task note (T8 review): `08` §3.2
(FR-123..125) substantively closes `01`'s OQ-002/OQ-003 crosscheck
family — occasional gratuities confirmed gravada, indemnización split per
Ley ISR Art. 4.3, sepelio no_gravada, illness/maternity subsidies gravada
— recorded as definitive stamps in 08; file 01's status rows are left
untouched pending wave review (annotated below).
...
- OQ-002 — Occasional gratuities ISR treatment: no corpus exclusion from remuneraciones gravadas; matrix marks gravada pending interfaces confirmation (closed in-wave by `08` §3.2 FR-125 — index note). open
- OQ-003 — ISR gravada classification of the non-vacation/aguinaldo prestaciones (indemnización, illness/maternity subsidies, sepelio) via the Ley ISR Art. 4 cross-check — the crosscheck_oq cells (resolved in-wave by `08` §3.2 FR-123..125 — index note). open
...
### 03_working-time-surcharges.md (4)
