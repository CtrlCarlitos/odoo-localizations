...
| 63_F930v3_informe_mensual_retIVA.pdf | F-930 v3 monthly retIVA inform homed in `fiscal-reporting/04`; the `fiscal-reporting/07` cite is reference-only (re-check cadence OQ-004). The former truncated source path in fiscal-reporting/04 LB-009 was fixed W26 (full registered stem; the scanner stem-rule fallback stays as a safety net only) |
