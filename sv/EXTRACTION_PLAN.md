...
- **OUTDATED CT COPY (blocking, found W5)**: our `05_Codigo_Tributario.pdf` lacks Arts. 119-A..119-H (DTE chapter), 239-A, and updated 206 — the entire e-invoicing legal base per Normativa v1.2 (Nov 2025). Obtain the CT reform (DTE/facturación electrónica law) before S1 synthesis.
- **Catalog v1.2 → v1.3+ drift (W5 confirmed)**: CAT-023/CAT-024 semantics affected by 18_ Normativa (contingency list, invalidation taxonomy); CAT-013 municipios pre-2023 reorganization. Fetch current catalogs + check CAT-024 still has "3 Otro".
