  ...
  **RECOVERED 2026-08-23** (DNS A-records restored — 168.90.109.76 +
   ...
   **uif addendum (owner-assisted, same day): the WAF block hits the
    ...
    **Post-W30 watch sweep (2026-08-24, same day; registry "External
    ...
    re-served BYTE-IDENTICAL (sha256), the hosted reglamento = a 6-pp
    D. 2-2000 print (17_-class, different scan) ⇒ NO new D.L.-426
    reglamento/instructivo evidenced; AML watch UNBLOCKED for
    monitoring (deadline 2026-10-17 stands). D.O. www + API serving
    but the feed STILL stalls at 10-ago-2026 (Ids 31815/31816
    unchanged; apex 000). MH: NO drift but ROUTE CHANGE — the apex
    host 302s to the www home swallowing paths (use
    `https://www.mh.gob.sv/…` for ALL MH probes); legacy wp-content
    deep links dead; the landing = the watch surface. factura NO
    DRIFT (wpdmdl set unchanged, max 5353). asamblea 200.**
