...
| `build_catalogs.py` | MH catalogs (sv) | `python build_catalogs.py <cc> <extracted_xlsx_txt>` — emits one CSV per catalog to `<cc>/requirements/catalogs/` + `_INDEX.md`. The workbook is the sole parse source; the official PDF is a human-reference fallback only, never a parse source (SV-CAT-FR-002). |
