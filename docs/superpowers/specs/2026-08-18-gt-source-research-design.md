...
| D-GT10 | Dated-instrument regime | Every normative/data artifact carries **valid_from/valid_to where printed** (law reforms with vigencia, salario mínimo AGs per year, FEL Reglas/catalog/XSD versions, form validities e.g. "SAT-2237 válido Sep-2013 en adelante", retention-agent rosters, certificador authorizations). Requirements must encode **date-driven rule selection**: the document's fecha de emisión determines which instrument versions apply — entering/backdating a document selects the then-in-force rules (rates, schemas, validation sets, forms) | Product owner raised 2026-08-19; **instantiates shared decision D16** (date-driven compliance mechanics: dated rule rows, rule-selection key, emission=certification timestamp with transmission-block on past dates + historical non-transmittable class, retro payroll with original-period rates, freeze-at-filing, backdating UX — recorded in `shared/docs/regulatory-change-management.md`, approved 2026-08-19; originally proposed as "D13", renumbered D16 at the 2026-08-19 cross-country merge because D13/D14 were taken by the journal/establishment-model decisions — see D16's header). EXTRACTION_PLAN reading units capture each instrument's vigencia data for the valid_from/valid_to sidecars |
