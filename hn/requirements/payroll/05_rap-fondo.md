...
| LB-016 | Ley RAP D.L. 107-2013, ORIGINAL Arts. 42/43/61 + 44-48 (W8 acquisition `120_`, G 33,222 6-sep-2013) | **The pre-reform statute layer (the doubly-load-bearing lead CLOSED):** ORIGINAL Art. 42: "Todas las empresas que cuenten con diez (10) o más empleados y aquellas que no estén inscritas a la fecha en el Régimen de Aportaciones Privadas (RAP) — Fondo Social Para la Vivienda (FOSOVI) —, estén obligadas a inscribirse e inscribir a sus trabajadores… a más tardar dentro de los sesenta (60) días siguientes a la publicación de esta Ley…; igualmente deben notificar… las nuevas contrataciones… o del cese de los mismos, dentro de los sesenta (60) días posteriores a la fecha de ocurrencia del nombramiento o cesantía." ORIGINAL Art. 43 (shared spans verbatim-identical to the reformed text): "El Patrono aportante debe retener el uno punto cinco por ciento (1.5%) del salario mensual ordinario de cada trabajador… dentro de los quince (15) días calendario siguientes a la retención"; "El Consejo Directivo con el voto favorable del sector productivo puede modificar el porcentaje de cotización tanto del trabajador como del patrono" — with **NO IVM-excess sentence and NO ceiling** (the IVM-excess base + the credits-retention final ¶ are 2024-reform insertions/appropriations); ORIGINAL Art. 61 kept the D. 167-91 special-regimes exception ("exceptuando lo referente a los Regímenes Especiales… Capítulo III, Sección IV del Decreto No.167-91… los cuales deberán proporcionar iguales o mejores condiciones") — DELETED by the 2024 reform. Arts. 44-48: inspector access; "Las aportaciones patronales no podrán ser deducidas en forma alguna del salario de los trabajadores" (Art. 45); recaudación delegable to the Sistema Financiero (Art. 46); 10-year prescription of unclaimed retiro balances (Art. 47); ISR treatment — interests exempt + aportaciones patronales/cotizaciones = gastos deducibles (Art. 48 ¶2, the employer-side deductibility leg LB-009 marked "not addressed"). Ejecútese 5-sep-2013 (Lobo) | `hn/sources/120_Gaceta_33222_DL107-2013_Ley_RAP.pdf` | Arts. 42-48 pp.8-9; Art. 61 p.11 (EV120:EVID-434..441; delta inventory EVID-437/438) |
...
- **HN-PAYR-FR-216:** (W8) The system shall carry the Ley RAP cotización
  regime as DATED rows across the 2013→2024 reform boundary (D-H2
  resolution = payslip period): (a) **valid 2013-09-06 → 2024-05-27**
  (original D.L. 107-2013): obligated class = *empresas con diez (10) o más
  empleados* + FOSOVI-unregistered employers, subject = "El Patrono
  aportante", worker 1.5% + employer 1.5% = 3% on the **FULL salario mensual
  ordinario (no IVM-excess rule, no ceiling)**, entero ≤15 días calendario
  following the retention, plus the D. 167-91 special-regimes exception
  (regimes providing equal-or-better conditions remain outside); (b)
  **valid from 2024-05-28** (D. 47-2024 reforms): universal "Todo patrono"
  registration, same 1.5/1.5 percentages (shared spans verbatim-identical),
  worker-side base = excess over the IHSS IVM techo (the inserted sentence),
  credits-retention ¶ appropriated from the Arts. 40-41 block, D. 167-91
  exception DELETED from Art. 61. Historical payslips/corrections in the
  2013-2024 window compute with the (a) rows; the RAP Consejo Directivo
  rate-modification power (unchanged across the reform) rides
  `watch_flag` on both row sets. (LB-011; LB-016;
  EV120:EVID-437/438; EV27:EVID-284/285)
...
|--------|-------|------|------------------|-----------|
| l10n_hn.rap.rate (new) | stream, rate_pct, base_rule, valid_from, valid_to, instrument, watch_flag | select/monetary/date/char/boolean | streams: fondo_employer (4%, base salario_ordinario, techo 3×SMM nivel más alto) · rap_worker (1.5%, base ivm_excess) · rap_employer (1.5%, base ivm_excess default, flag); all valid_from 2024-05-28; watch_flag for RAP Consejo Directivo instruments; never replaced in place (D-H2). W8 pre-reform set (FR-216): rap_worker/rap_employer 1.5% valid 2013-09-06→2024-05-27, base = full salario mensual ordinario, obligated_class = empresas_10_plus + FOSOVI-unregistered, exception = D.167-91 regimes | FR-184, FR-186..FR-188, FR-191, FR-216 |
...
| l10n_hn.rap.base.flag (new) | flag, hypothesis, status | select/char | salario_ordinario_undefined (CT-ordinary, 13th/14th excluded — hypothesis only) · art43_universality [RESOLVED W8: universality = the 2024 reform, LB-016] · patronal_base_scope [NARROWED W8: pre-2024 = full base for BOTH sides; post-2024 worker-side excess literal, patronal default same — flag retained] · pre_reform_values_unknown [RESOLVED W8: original = same 1.5/1.5, LB-016]; flags never inject values into config | FR-193, FR-195, FR-216 |
...
| FR-212..FR-214 | odoo | hr.employee exceptions + l10n_hn.rap.notification | eligibility + SLA | 60-day notification SLAs fed by hire/termination events; transcription completeness W8-verified (OQ-006 resolved); diplomatic convenio = conditional flag |
...
- **AC-019 (W8):** Given a May-2024 payslip for a worker with salario
  ordinario L20,000 and a 2024 IVM techo of L11,336.32, then the RAP pair
  resolves with the ORIGINAL-Art.-43 rows — worker 1.5% and employer 1.5%
  both on the FULL L20,000 (L300.00 each, no excess rule); given the same
  worker in June-2024 (post-reform), the worker 1.5% base becomes the
  L8,663.68 excess (L129.96) with the patronal base defaulting to the same
  excess under its retained flag — the dated-row switch happens exactly at
  the 2024-05-28 boundary and historical May-2024 corrections recompute
  with the original rows (FR-216; FR-186..FR-188).
...
| OQ-003 | ~~Reformed Ley RAP Art. 43 scope~~ **RESOLVED W8** (R-H85; `120_` acquired, LB-016/EV120:EVID-434..441): (i) universality = the 2024 reform's change (original = 10-employee + FOSOVI-unregistered gate; subject "El Patrono aportante"); (ii) IVM-excess base = a 2024 INSERT governing "La retención" (worker side); pre-2024 both sides = full salario mensual ordinario; post-2024 patronal default = same excess (flag retained — the reform text is worker-literal); (iii) pre-reform values = the SAME 1.5/1.5 (shared spans verbatim-identical). Encoded FR-216. | no | controller W8 | resolved |
...
| OQ-006 | ~~Art. 16 transcription integrity~~ **RESOLVED W8** (`120_` delta inventory, EV120:EVID-437): the reformed texts are COMPLETE — 47_'s printed ellipsis sits exactly where the original Art. 42 ends ("…nombramiento o cesantía."); nothing is missing from the transcription. The 60-day publication reference = D. 47-2024's own publication (the reform's operative frame), ~27-jul-2024 initial window stands. | no | controller W8 | resolved |
