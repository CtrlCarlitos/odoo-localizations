...
HN-EINV-FR-139/140); and the SAR-236-2024 instrument itself (unacquired —
top acquisition lead; all its ordinals are quoted only as reproduced inside
the corpus manuals).
...
|----|--------------------|---------------------|-------------|----------|
| LB-001 | Acuerdo SAR-236-2024 (UNACQUIRED — LEAD), PRIMERO/SEGUNDO/TERCERO a-b + DÉCIMO SÉPTIMO, as quoted in every Ayuda — PRIMERO: "Se crea la Oficina Virtual… como la herramienta que facilita a los obligados tributarios el cumplimiento voluntario…"; SEGUNDO: "Implementar la plataforma digital denominada 'Oficina Virtual', como único medio…"; TERCERO a/b define "Acuse Electrónico" and "Boletín de Pago" (with caducidad date); DÉCIMO SÉPTIMO: SW modality via "la plantilla proporcionada por el Servicio de Administración de Rentas (SAR)" | OVI creation as the voluntary-compliance platform; único-medio doctrine; electronic-acknowledgment and payment-slip definitions (slip carries an expiry date); SW plantilla modality — the chassis instrument, quoted family-wide | `hn/sources/43_Ayuda_ISV_201.pdf` (quotes in every Ayuda §I) | PRIMERO..TERCERO, DÉCIMO SÉPTIMO as quoted (EV31:EVID-128; EV43:EVID-136) |
...
| LB-019 | Stale-channel/deadline guards: 72_ §4, 45_/72_ "8 días", 71_ §4 — 72_ §4: "El único medio de presentación de esta Declaración es Oficina Virtual: https://oficinavirtual.sar.gob.hn/" (stale); 45_: "dentro de los primeros ocho (8) días calendario … de conformidad con el Acuerdo N.º SAR-237-2024" while 237's own text says "cinco (05)"; 71_ §4: "El único medio… Oficina Virtual" (overbroad — 523 is SW-only) | R-H27/R-H18 dead-text guards: single-channel sentences and the 8-day deadline are NEVER loadable as config; gazette text and SAR-236-2024 DÉCIMO SÉPTIMO/OCTAVO + SAR-237-2024 2.II govern | `hn/sources/72_Generalidades_DMC.pdf` + `hn/sources/45_Ayuda_DMC.pdf` + `hn/sources/71_Generalidades_DMR.pdf` | 72-§2/§4; 45-§I.5 p.6; 71-§4 p.8 (EV43:EVID-143/144; EV31:EVID-125) |
...
| OQ-008 | SAR-236-2024 ordinal structure (origin `66_ OQ-5`, VERIFY; kin `65_ OQ-5`): 66_ cites "sección séptima/octava" in an odd ordinal style while others cite ordinals (PRIMERO…DÉCIMO NOVENO); the Décimo Tercero informativa catalog (item 29 = 542; items 30-31 truncated in print) is sighted only via 65_ — full ordinal map + catalog pinned only on acquisition (same instrument as OQ-001/OQ-006). | no | acquisition queue | open |
| OQ-009 | SAR-007-2017 residual status (file-local, VERIFY): evidenced only as the 2017 transitional format-keeper for CPAT-era declarations (EV13:EVID-080); whether any format obligation survives SAR-238-2024 for pre-2024 rectifications is unknown — historical import resolves to DMR rows regardless (R-H31). | no | Takumi S-HN3 | open |
